...
const navLinkClasses = (active) =>
  `rounded-md px-3 py-2 text-sm font-medium transition-colors ${
    active ? 'bg-indigo-600 text-white' : 'text-slate-200 hover:bg-indigo-500/80'
  }`;

const SiteHeader = () => {
  const { user, isAuthenticated, isAdmin, isStudent, logout, processing } = useAuth();
    ...
    <>
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
        ...
        <nav className="flex items-center gap-2">
          ...
          {isStudent && !isAdmin && (
            <Link to="/student" className={navLinkClasses(location.pathname === '/student')}>
              Student Dashboard
            </Link>
          )}
          {isAdmin && (
            <Link to="/admin" className={navLinkClasses(location.pathname === '/admin')}>
              Admin Dashboard
            </Link>
          )}
        </nav>
        <div className="flex items-center gap-3 text-sm">
          {canShowLogout ? (
            <>
              <span className="hidden text-slate-200 sm:inline">Hi, {displayName}</span>
              <button
                type="button"
