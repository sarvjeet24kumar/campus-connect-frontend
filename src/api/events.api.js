...
export const getEvents = async (params = {}) => {
  const response = await eventsClient.get('/events/', Object.keys(params).length ? { params } : undefined);
...
export const softDeleteEvent = async (eventId) => {
  const response = await eventsClient.post(`/events/${eventId}/soft_delete/`);
  return response.data;
};
